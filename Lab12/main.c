...
void insert_node(int x, struct heap_struct * H)
{
    ...
    else
    {
       ...
       H->crnt_heap_size++;
        H->elements[H->crnt_heap_size] = x;
        i = H->crnt_heap_size;
        while(H->elements[i/2] < H->elements[i] && i > 1)
        {
...
void delete_root(struct heap_struct * H)
{
    ...
    {   printf("\nHeap is empty. ERROR");
        return;}
    int lastElement= H->elements[H->crnt_heap_size];
    H->elements[1]= lastElement;
    H->crnt_heap_size--;
    int i =1;
    while(i*2 <= H->crnt_heap_size)
    {
        int child=i*2;
        if ((H->elements[i]< H->elements[child]) && (child<=H->crnt_heap_size))
        {
            swap_ref(&H->elements[i], &H->elements[child]);
            i=child;}
        else if((H->elements[i] < H->elements[child+1]) && (child+1 <= H->crnt_heap_size))
        {
            swap_ref(&H->elements[i], &H->elements[child+1]);
            i=child+1;}
        else
            break;  }
    return; }
