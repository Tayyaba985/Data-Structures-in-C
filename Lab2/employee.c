...
/*** This function searches for a record in the list by matching age.
*/
int searchNodeByAge(struct employee * head, int age)
{
    ...
    while(((temp->age) != age) && ((temp->next) != NULL))
    {
        temp = temp->next;
        index ++;
    }
    if((temp->next == NULL) && ((temp->age) != age))
        return(-1);
...
int deleteNodeByName(struct employee * head, char * name)
{

    struct employee * temp = head;
    int index = 0;
    struct employee * pre = temp;
...
int deleteNodeByAge(struct employee * head, int age)
{
    /*** Insert your code here ***/
    struct employee * temp = head;
    int index = 0;
    ...
    if((temp->next == NULL) && ((temp->age) != age))  /// Age not found
        return(-1);
    pre->next = temp->next;
    free(temp);
    return (index);
