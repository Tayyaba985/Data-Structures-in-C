...
int main()
{
    ///****************** This section generates the grid containing 1s and 0s *********************//
    ...
    for(int i=0; i<ROWS; i++)
    {
        ...
        {
            my_grid[i][j] = ((rand() % 10) > 6);    /// Initialize the grid with randomly placed 0 and 1
...
int find_shortest_path(int * my_grid, int num_rows, int num_cols, int src_x, int src_y, int dst_x, int dst_y)
{
    ...
    {
        current_visiting = q_delete(&qfront, &qrear);   /// Step 1: remove the node at the front
        ...
        if((visiting_x == dst_x) && (visiting_y == dst_y))    /// found the destination
            break;
        else        /// Add the neighbors with cost added into the queue
        {

            for(int i=0; i<4; i++)      /// Visiting the 4 possible neighbors.
            {
