...
int test_palindrome_rec(char * test_word);   /// Complete This
#endif // TASK1
...
int main()
{
        ...
        printf("\nRecursive Output: ");
        if((test_palindrome_rec(my_word))==1)
            printf("This word is a Palindrom!\n");
        else
            printf("This word is not a Palindrom!\n");
...
int test_palindrome_rec(char * test_word)
{
    /** Complete this function **/
    /// You may change the prototype of the function to include more arguments.
 int i,c=0,n=strlen( test_word);
    if(i<n/2)
    {
         if(test_word[i]==test_word[n-i-1])
    	c++;
    	i++;
    	test_palindrome_rec(test_word);
	}
	else
	{
		if(c==i)
