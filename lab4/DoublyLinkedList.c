...
void  insertNodeAtEnd(struct node_d ** head)
{
   ...
   else
   {
        ...
        new_node->prev = temp;
        (*head)->prev= new_node;    /// Point to the second last item
   }
...
int searchNodeByData(struct node_d * head, int field_num)
{
    ...
    switch(field_num)
    {
        ...
        case EMP_AGE:
        {
            ...
            if(idx == -1)
                printf("\nAge searched for is not found!!\n");
            else
                printf("\nThe age exists at index %d", idx);
        ...
        case EMP_BS:
        {
            ...
            if(idx == -1)
                printf("\nBasic salary searched for is not found!!\n");
            else
                printf("\nBasic salary name exists at index %d", idx);
            break;
        }
    }
    return(idx);
...
int  deleteNodeAfter(struct node_d * head, int idx)
{
   ...
   struct node_d *temp=head;
   struct node_d *del=temp->next;
   struct node_d *aft=del->next;
    if(getListLength(head)==0 || idx ==0)
    {
        printf("\n there's no node for deletion\n");
        return(-1);
    }
...
void printMemMap(struct node_d * head)
{
   ...
   {
       printf("%d\t%u\t%d\t%u\t%u\n",count,temp,temp->data.age,temp->next,temp->prev);
       temp=temp->next;
